...
function display(){
    var handle = document.getElementById('input').value;
    console.log(handle);
 ...
 function work(data){
    ...
    if(title[1].innerText=='undefined') title[1].innerText='No name found on codeforces';
    ...
    $.ajax({
     ...
     function problems(data){
        var ans=0;
        var acnt=0, bcnt=0, ccnt=0, dcnt=0, ecnt=0,fcnt=0,gcnt=0,hcnt=0,icnt=0,jcnt=0,tcnt=0;
        var prob = new Set([]);
        var index = new Set([]);
